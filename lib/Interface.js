export default class Interface {
    ...
    static editBondDlg(that) {
        if (that.bondslist.length > 0) {
            ...
            this.modal(text, that.parent, that.addBonds.bind(that), items, "table", fixedBonds, that)//, that.chainname);//that.bondslist);
            that.chaindict = chaindict;
            that.chainname = chainname;
        }
    }
    ...
    static modal(title, parent, submitFunction, items, options, fieldData, that) {
        var toAttach = []
        ...
        if (!items) {
            ...
            submitButton.addEventListener("click", () => { this.tableButtonEventListener(toAttach, submitFunction, fieldArray, body, that) });
        } else {
    ...
    static tableButtonEventListener(toAttach, submitFunction, fieldArray, body, that) {
        var data = [];
        let columnNames = {};
        let overlay = body.querySelector(".modal-js-overlay");
        //that.setEmptyData();
