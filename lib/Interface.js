import { getDictKeys } from './Tools.js';
...
export default class Interface {
    ...
    static addBondDlg(that) {
        ...
        var types = that.types ? getDictKeys(that.types) : [];//fetched from config or admin menu (code exists to add/edit types but I was told to shelf it (remove))
        ...
        const items = {
            ...
            bondtype: {
                label: "Bond Type", key: "bondtype", type: "select", items: types
            },
    ...
    static editAnnotationDlg(that) {
        if (that.annotationList.length > 0) {
            ...
            this.modal(text, that.parent, that.addAnnotations.bind(that), items, "table", that.annotationList, that, "annotation");
            that.chainname = chainname;
            that.chaindict = chaindict;
        }
    }
    ...
    static editBondDlg(that) {
        if (that.bondslist.length > 0) {
            ...
            var types = that.types ? getDictKeys(that.types) : [];

            let chainname2 = chainname;
            chainname2.undefined = "None";

            const items = {
                seq: { label: "First Sequence", key: "seq", type: "select", items: chainname },
                firstpos: { label: "First Position", key: "first", type: "input", width: 100 },
                secondseq: { label: "Second Sequence", key: "secondseq", type: "select", items: chainname },
            ...
            for (i in that.bondslist) {
                ...
                newBond.bondtype = that.bondslist[i].bondtype;
            ...
            this.modal(text, that.parent, that.addBonds.bind(that), items, "table", fixedBonds, that, "bond")//, that.chainname);//that.bondslist);
            that.chaindict = chaindict;
            that.chainname = chainname;
        }
    }
        ...
        for (const [key, value] of row) {
            ...
            if (rowType.type == "color") {
                ...
                for (var j in rowType.items) {
                    ...
                    if (rowType.key == "bondtype")
                        option.value = rowType.items[j];
                    else
                        option.value = j;
                    option.text = rowType.items[j];
                    if (value == j || rowType.items[j] == value)
                        //if(chainname[value] == option.text)
                        option.selected = true;
                    else if (!value && value != 0 && j == "undefined")
                        option.selected = true;
    ...
    static modal(title, parent, submitFunction, items, options, fieldData, that, type) {
        var toAttach = []
        ...
        if (!items) {
            ...
            for (const [key, value] of Object.entries(fieldData)) {
                ...
                order.forEach((columnName) => {
                    //                    if (!value[columnName] && columnName == "secondseq")
                    sortedValues.push([columnName, value[columnName]]);
                })
                this.addRow(table, sortedValues, fields)//, chainname);
            }
            
            submitButton.addEventListener("click", () => { this.tableButtonEventListener(toAttach, submitFunction, fieldArray, body, that, type) });
        } else {
    ...
    static tableButtonEventListener(toAttach, submitFunction, fieldArray, body, that, type) {
        var data = [];
        let columnNames = {};
        let overlay = body.querySelector(".modal-js-overlay");
        //that.setEmptyData();
        ...
        if (type == "bond")
            that.setEmptyBondData();
        else if (type == "annotation")
            that.setEmptyAnnotationData();
        that.ensureBondObjects();
