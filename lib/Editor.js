...
import { parseXml, arrayMax, createButtons } from './Tools.js';
...
export default class Editor {
    ...
    addBonds(data, fromXml) {
        ...
        for (let j = 0; j <= Math.max(...this.editorIndices); j++) {//cycling through and clearing stored bond information from all editor indices that have been used (including removed), so that they can be replaced with the new incoming data (which is an edited version of the old data)...
            ...
            if (data.length == 0) {//edge case where bond doesn't dissappear after all bonds taken away!!!
                this.editors[index].clearEditor();
            }
        }
    ...
    drawBonds(removedBonds) {
        ...
        for (let l = 0; l < this.editorIndices.length; l++) {
            ...
            this.outerHover();
            let solo = this.editors[index].singlearr;//TIME COMPLEXITY BOTTLENECK!!!
    ...
    deleteChain(index) {
        if (this.currentSelected) {
            let keys = Interface.getDictKeys(this.editors[this.currentId].singlebonds);
            let removedBonds = {};
            for (let i in keys) {
    ...
    buildSVG(col) {
        ...
        let add = maxAdd >= 50 ? maxAdd : 50;//if still having problems just put add as maxAdd as it appears to be working great in test environment...
