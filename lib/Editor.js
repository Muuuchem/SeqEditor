...
export default class Editor {
    ...
    outerHover() {
        for (var i = 0; i < this.outer.length; i++) {
            ...
            var title = 'Bond between ' + bond.seq + ' at position ' + firstPos + ' and sequence ' + bond.secondseq + ' at position ' + secondPos + ':' + notes;
            span.title = title;
        ...
        while (length >= 0 || current != undefined) {
            ...
            if (current == '&') {//case where we have &nbsp;
                ...
                while (current != '>') {
                    ...
                    }// else if (current == '>' && openAnnotation == true && sequence.slice(n - 6, n) == "</span>") {
                    //    console.log("err");
                    //}
                    span += current;
                    n++;
