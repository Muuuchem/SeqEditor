export default class SequenceEditor {
    ...
    addEditorEventListeners(parent, paragraph, that) {
        ...
        paragraph.addEventListener("focusout", function (e) {
            ...
            if (paragraph.innerHTML != me.beforeEdit) {
                var length = me.quickParse(paragraph.innerText).length;
                var input = that.extractInput(paragraph.innerHTML, length, me.num);
    ...
    addHoverEvents2(inner) {
        for (var i = 0; i < inner.length; i++) {
            ...
            var seq = data.seq;
            var spanOne = document.getElementById("$" + this.num + ":" + data.id);
            ...
            if (!spanOne || !spanTwo)
                continue;
            var title = 'Bond within' + seq + ' from ' + firstPos + ' to ' + secondPos + ': ' + note;
            spanTwo.title = title;
            spanOne.title = title;
        }
    }
